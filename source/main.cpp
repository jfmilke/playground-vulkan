...
void HelloTriangleApp::cleanUp() {
  ...
  // necessary here if viewport & scissor dynamic
...
void HelloTriangleApp::createRenderPass() {
  ...
  depthAttachmentRef.attachment = 1;  // index of attachment description
...
void HelloTriangleApp::createFramebuffers() {
  ...
  for (size_t i = 0; i < swapChainImageViews.size(); i++) {
    std::array<VkImageView, 3> attachments = {
        colorImageView
      , depthImageView // one depth bc only 1 subpass is running at a time (semaphore)
...
void HelloTriangleApp::createTextureImage() {
  ...
  // "log2" to check how many times dims can be divided by 2 and "+1" to save space for original image
...
void HelloTriangleApp::generateVertexData() {
  // Triangle
  /*
  vertexData = {
...
void HelloTriangleApp::pickPhysicalDevice() {
  ...
  for (const auto& device : devices) {
    if (isDeviceSuitable(device)) {
      ...
      msaaSamples = findMaxUsableSampleCount();
      break;
    }
  }
...
bool HelloTriangleApp::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  bool suitable = indices.isComplete() && extensionsSupported && swapChainAdequate && deviceFeatures.samplerAnisotropy;
...
int HelloTriangleApp::rateDeviceSuitability(VkPhysicalDevice device) {
  ...
  // GPU needs to be capable of anisotropic filtering
...
VkSampleCountFlagBits HelloTriangleApp::findMaxUsableSampleCount() {
...
void HelloTriangleApp::recreateSwapChain() {
  ...
  // don't necessary here if viewport & scissor are dynamic
  // createGraphicsPipeline();
...
void HelloTriangleApp::loadModel() {
  ...
  if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, "../data/meshes/viking_room.obj")) {
    throw std::runtime_error(warn + err);
  }

  std::unordered_map<Vertex, uint32_t> mapVertexToIndex{};
  ...
  for (const auto& shape : shapes) {
    for (const auto& index : shape.mesh.indices) {
      ...
      v.texCoord = {
          ...
          // flip this into "top to bottom" representation
        , 1.0f - attrib.texcoords[2 * index.texcoord_index + 1]
      };

      v.color = { 1.0f, 1.0f, 1.0f };

      if (mapVertexToIndex.count(v) == 0) {
        mapVertexToIndex[v] = static_cast<uint32_t>(vertexData.size());
        vertexData.push_back(v);
      }

      indices.push_back(mapVertexToIndex[v]);
    }
  }
}
